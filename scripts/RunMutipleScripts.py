...
def CheckMutiple_Scripts(Path, **kwargs):
    ...
    def success(file):
        ...
        with open(file,"r") as f:
            content = f.readlines()
            if len(content) > 1:
                if not "Merginguntagged" in content[-1]:
                    print file
                else:
                    good = True
            else:
...
Path = "/stash/user/qilongguo/work/gKK/VVVnano/condor/V1/WrongBranches/"
Scripts = "/stash/user/qilongguo/work/gKK/VVVnano/condor/V1/2.sh"
